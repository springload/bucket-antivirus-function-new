...
archive: clean  ## Create the archive for AWS lambda
	...
	mkdir -p ./build/
	docker run --platform linux/amd64 -v $(current_dir)/build:/opt/mount --rm \
    --entrypoint cp bucket-antivirus-function:latest /opt/app/build/anti-virus.zip /opt/mount/anti-virus.zip
